"""Module extensions for rules_codex (bzlmod)."""
...
_TOOLCHAIN_BUILD_HEADER = """# Generated by rules_codex

load("@rules_codex//codex/private:toolchain.bzl", "codex_toolchain")
...
toolchain(
    ...
    toolchain_type = "@rules_codex//codex:toolchain_type",
)
...
toolchain(
    ...
    toolchain = ":{platform}_impl",
    toolchain_type = "@rules_codex//codex:runtime_toolchain_type",
)
"""
...
def _find_modules(module_ctx):
    """Find the root module and rules_codex module.
    ...
    for mod in module_ctx.modules:
        if mod.is_root:
            root = mod
        if mod.name == "rules_codex":
            rules_codex = mod
    if root == None:
        root = rules_codex
    if rules_codex == None:
        fail("Unable to find rules_codex module")
    return root, rules_codex

def _codex_impl(module_ctx):
    """Implementation of the codex module extension."""
    root, rules_codex = _find_modules(module_ctx)

    downloads = root.tags.download or rules_codex.tags.download
